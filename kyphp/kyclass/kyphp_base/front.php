...
final class front {
	...
	public function __destruct(){
		 ...
		 if($this->kyphp->config->get('debug')=='on'){
			if(!empty($GLOBALS['debug_class_time']))foreach($GLOBALS['debug_class_time'] as $classname=>$time){
				echo "<br>Time: The class $classname spent is $time (ms)";
			
			}
			if(isset($GLOBALS['debug_spent_time']))echo "<div>Time: Total spent is ".$GLOBALS['debug_spent_time']." (ms)</div>";
		 }
