...
final class KYPHP {
	...
	public static function Run($config=array()){
		...
		if($KYPHP->config->get('templete_mode')=='smarty'){
			$smartyClass=$KYPHP->config->get('smarty_library').'/Smarty.class.php';
			if(file_exists($smartyClass)){
				require $smartyClass;
				require_once KYPHP_PATH.'kyclass/kyphp_base/Smarty.php';
				
			}else{
				trigger_error('Error: smarty class not exists :'.$smartyClass);
	...
	static function autoload_APP($class,$locdirs){
		...
		if($locdirs){
			...
			if(isset($config['DEFAULT_LIB_PATH']))
			{
				$controlclassfile=APP_PATH.DIRECTORY_SEPARATOR.$config['app_dir'].$config['DEFAULT_LIB_PATH'].'/'.str_ireplace('Action','',strtolower($class)).'.class.php';	
			...
			if(isset($config['DEFAULT_C_PATH']))
			{
				$controlclassfile=APP_PATH.DIRECTORY_SEPARATOR.$config['app_dir'].$config['DEFAULT_C_PATH'].'/'.str_ireplace('Action','',strtolower($class)).'.class.php';	
