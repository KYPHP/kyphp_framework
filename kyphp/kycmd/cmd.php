...
$content1='';
  foreach($volist[$name] as $key=>$k){
    $$id=$k;
	  ...
	  $content2=preg_replace("/[{][$](\w+)[}]/eis",'checkArrayindex($k,\'\\1\')',$content);
...
$customcmd= APP_PATH.'/cmd/cmd.php';
if(is_file($customcmd)){
	require_once($customcmd);
...
function php_exe($string,$obj,$data){
	$string=preg_replace('/<\?php(.*?)\?>/eis','php_exe_back(\'\\1\',$obj,$data)',$string);
	
	return $string;
}
function php_exe_back($cmd,$obj,$data){	
	extract($data);	
	ob_start();
	
	eval(str_replace('$this','$obj',stripslashes($cmd)));
	$out = ob_get_contents();	
    ob_end_clean();
