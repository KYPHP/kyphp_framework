...
$_charset=require_once(KYPHP_PATH.'kylang/'.__CHARSET__.'/default.php');
...
{ 
	...
	if($error_display=='on'){
		...
		kyphp_display_error($error,$errno, $errstr, $errfile, $errline,$kyphp_error_no);		
		$kyphp_error_no++;
	}
...
function kyphp_display_error($error,$errno, $errstr, $errfile, $errline,$kyphp_error_no_get){	
	global $_charset,$KYPHP;
	
	if($kyphp_error_no_get==0){
		echo '<!DOCTYPE html PUBLIC "-//W3C//DTD XHTML 1.0 Transitional//EN" "http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd">';
		echo "<htmL><head><title>{$_charset['error']} ($error) $errstr</title>";
	...
	echo "<b>{$_charset['error_info']}:</b>($error) {$_charset['error_no']}[$errno] $errstr {$_charset['file']} $errfile ".sprintf($_charset['lineno'],$errline)."<br />";
...
function showdebuglist($debug){
	global $_charset,$KYPHP;
	echo '<ul>';
	foreach($debug as $key=>$value){
		if(0===strcmp($value['function'],'kyphp_display_error'))continue;
		if(0===strcmp($value['function'],'debugtrace'))continue;
		$classname='';	
		$time='';
		...
		echo "<li>{$_charset['file']}{$value['file']} ".sprintf($_charset['lineno'],$value['line'])." {$classname} {$value['function']} {$time}
